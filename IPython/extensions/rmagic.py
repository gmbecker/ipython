...
class RMagics(Magics):
    ...
    def caching_eval(self, line, cache_random):
        ...
        No caching is done if code with random elements is detected. 
        Randomness is detected by comparing the .Random.seed R variable
        before and after code evaluation. 

        This will detect code leveraging most basic methods of generating 
        random numbers (rnorm, runif, etc), but is not guaranteed to detect
        more exotic RNGs, such as those implemented in non-core R packages
        which don't make use of .Random.seed. See ?set.seed in R
